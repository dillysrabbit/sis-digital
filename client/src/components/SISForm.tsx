...
export function SISForm({ initialData, onSave, onGeneratePlan, onCheckSis, isSaving, isGenerating, isChecking }: SISFormProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-l-4 border-l-blue-500 pt-0 gap-0">
        <CardHeader className="bg-blue-500 text-white rounded-t-xl py-3">
      ...
      {/* Feld A - O-Ton */}
      <Card className="border-l-4 border-l-[var(--sis-oton)]] pt-0 gap-0">
        <CardHeader className="sis-oton text-white rounded-t-xl py-3">
      ...
      {/* Themenfelder */}
      {themenfelder.map((tf) => (
        <Card key={tf.key} className={`border-l-4 pt-0 gap-0`} style={{ borderLeftColor: `var(--${tf.color.replace("sis-", "sis-")})` }}>
          <CardHeader className={`${tf.color} ${tf.textColor} rounded-t-xl py-3`}>
